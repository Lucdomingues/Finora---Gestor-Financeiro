...
function TransactionItem({ elements, handleDelete, handleUpdate }) {
  const [updateIsTrue, setUpdateIsTrue] = useState(false);

  const updatedForm = (obj) => {
    setUpdateIsTrue(!updateIsTrue);
    if (obj) {
      handleUpdate(obj);
    }
  };
  return (
    <>
      {updateIsTrue === true ? (
        <TransactionFormUpdate elements={elements} updatedForm={updatedForm} />
      ) : (
        <>
